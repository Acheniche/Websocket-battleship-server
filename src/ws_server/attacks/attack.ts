import { Field, MAP_SIZE, Position, ShipStatus, wsUser, rooms } from '../types/types';
  ...
  export const attack = (
    ...
    const { ships, killed, shots } = indexPlayer === 1 ? field.firstUserShipsData : field.secondUserShipsData;
    ...
    const { killedDraft, shipsDraft, status, data } = attackShip( x, y, ships, killed, shots );
    if (!indexPlayer) {
  ...
  const initEdgeCoors = (killed: Position[] | undefined) => {
    ...
    const filtered = edgeCoors.filter(
      (coor) => coor.x >= 0 && coor.x <= MAP_SIZE && coor.y >= 0 && coor.y <= MAP_SIZE,
    );
    const data = { filtered, killed };
    return data;
  };
  
  const sendTurn = (field: Field) => {
  ...
  const finishGame = (id: string, field: Field) => {
    const users = [field.firstUser, field.secondUser];
    users.forEach((user) =>
      user.send(
        JSON.stringify({
